	...
	public void onUpdate(EventInteract event) {
		...
		try {
			...
			if (mc.player.getVehicle() != null) {
				...
				mc.player.connection.send(new ServerboundMovePlayerPacket.Pos(previouspos.get(Direction.Axis.X), previouspos.get(Direction.Axis.Y), previouspos.get(Direction.Axis.Z), false, mc.player.horizontalCollision));
				// Do it again to be sure it happens
				mc.player.connection.send(new ServerboundMovePlayerPacket.Pos(previouspos.get(Direction.Axis.X), previouspos.get(Direction.Axis.Y), previouspos.get(Direction.Axis.Z), false, mc.player.horizontalCollision));
